...
export const decisionSchema=(count:number)=>({type:'object',additionalProperties:false,properties:{decisions:{type:'array',minItems:count,maxItems:count,items:{type:'object',additionalProperties:false,properties:{id:str,fit:{type:'string',enum:['strong','possible','none']},relation:str,why:str,talk:str,gap:str,score:{type:'integer',minimum:0,maximum:100},refs:{type:'array',items:{type:'object',additionalProperties:false,properties:{field:{type:'string',enum:['intro','needs']},quote:str},required:['field','quote']}}},required:['id','fit','relation','why','talk','gap','score','refs']}}},required:['decisions']});
